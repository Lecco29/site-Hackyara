...
const ShapeGrid = ({
  ...
  useEffect(() => {
    ...
    const updateAnimation = () => {
      const effectiveSpeed = reduzMovimento ? 0 : Math.max(speed, 0.1)
    ...
    const cellFromMouse = (mouseX, mouseY) => {
      if (isHex) {
        const colShift = Math.floor(gridOffset.current.x / hexHoriz)
        const offsetX = ((gridOffset.current.x % hexHoriz) + hexHoriz) % hexHoriz
        const offsetY = ((gridOffset.current.y % hexVert) + hexVert) % hexVert
    ...
    const handleMouseMove = (event) => {
      ...
      const cell = cellFromMouse(event.clientX - rect.left, event.clientY - rect.top)
    ...
    // escuta na seção que contém o canvas: o conteúdo sobreposto não bloqueia o hover
    const alvo = canvas.closest('section') || canvas
    alvo.addEventListener('mousemove', handleMouseMove)
    alvo.addEventListener('mouseleave', handleMouseLeave)
    ...
    return () => {
      ...
      alvo.removeEventListener('mousemove', handleMouseMove)
      alvo.removeEventListener('mouseleave', handleMouseLeave)
    }
